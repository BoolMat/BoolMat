...
\begin{defi}
  Given vector $v \in \B^n$ and permutation $p \in S_n$, the \emph{action on
  sets} of $p$ on $v$, denoted by $v \cdot p$, is given by the boolean
  vector containing a $1$ in position $(i)p$ if and only if component $i$ of $v$
  is $1$.
\end{defi}
The name \emph{action on sets} simply arises from considering a vector $v \in
\B^n$ as a characteristic function on $\{1 \ldots n \}$. 
There are also two actions of $S_n$ on $\Bn$, given by permuting the rows and
columns of the matrix. A row permutation is equivalent acting on the columns as
sets, and vice versa. These are the only actions we consider of $\S_n$ on
$\Bn$, so we will often omit the name of the action in the following.
